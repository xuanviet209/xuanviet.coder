...
$errors = [];
// gán mặc định
$studentName = $studentPhone = $studentEmail = $studentBirth = $studentGender = $studentNote = $studentCity =$studentWebsite =  $studentPoint = $studentSchoolMove = "";
...
if (isset($_POST) & !empty($_POST)) {
    ...
    if(!isset($_POST["phone"]) || empty($_POST["phone"])) {
        $errors["name"] = "Bắt buộc phải nhập số điện thoại sinh viên";
    } else {
    ...
    //     $errors["website"] = "Bắt buộc phải nhập website ";
    // } else {
    //     $studentWebsite = trim($_POST["website"]);
    //     $patternStudentWebsite = "/^([a-zA-Z0-9]{3,})@([a-zA-Z0-9]{3,})/i";
    //     $studentWebsiteValidate = preg_match_all($patternStudentWebsite, $studentWebsite, $matches);
    //     if (!$studentWebsiteValidate) {
    //         $errors["website"] = "website  không đúng";
    //     }
    // }
    //diem
    // if(!isset($_POST["point"]) || empty($_POST["point"])) {
    //     $errors["point"] = "Bắt buộc phải nhập point ";
    // } else {
    //     $studentPoint = trim($_POST["point"]);
    //     $patternStudentPoint = "";
    //     $studentPointValidate = preg_match_all($patternStudentPoint, $studentPoint, $matches);
    //     if (!$studentPointValidate) {
    //         $errors["point"] = "point  không đúng";
    //     }
    // }
...
<body>
    ...
    <form name="profile" method="post" action="">
        ...
        <div>
           ...
           <!--   <?php
            if (isset($errors["website"]) && $errors["website"]) {
                echo "<p style='color:red'>".$errors["website"]."</p>";
            }
            ?> -->
        </div>
        ...
        <div>
           ...
           <!--  <?php
            if (isset($errors["point"]) && $errors["point"]) {
                echo "<p style='color:red'>".$errors["point"]."</p>";
            }
            ?> -->
        </div>
